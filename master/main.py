...
def game(current_move='KEY_RIGHT'):
    ...
    while True:
        ...
        #GET BEST MOVE
        best_move = get_best_move(current_move=current_move, available=available_moves, dx=distanceX, dy=distanceY)
...
def get_best_move(current_move='', available=[], dx=0, dy=0):
    ...
    if fruit_move_head in snake or get_free_space(move=best_move) < random.choice([3,4,5,6,7,8,9,10]):
        max_score = 0
...
def get_free_space(move=''):
    ...
    while possible_head not in snake and alive < (window.getmaxyx()[1] and window.getmaxyx()[0]): #se nao for instant death
        ...
        possible_head = teleport_snake_head(head=possible_head)
